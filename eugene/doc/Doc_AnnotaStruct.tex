...
The sensor is activated by using the value 1 for the parameter
...
\texttt{AnnotaStruct.format} in the parameter file.
The plugin reads a collection of similarities from files named with
the sequence name and an extension \texttt{AnnotaStruct.FileExtension} plus \texttt{.gff3}
In gff3 mode you can't describe feature level as in natif mode.

If you want to read score or probability from file you have to set parameters score to "i".
Content data will be interpreted as score.
\begin{Verbatim}[fontsize=\small]
AnnotaStruct.Exon*            i
...
Here is the correspondance beetween AnnotaStruct feature and gff3 features (note that low level content can't be describe directly, but by setting signal at zero, and using transcribe sequence feature you can simulate it)
