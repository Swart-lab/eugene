...
\author{Erika Sallet \and J\'er\^ome Gouzy \and Philipe Bardou \and Marie-Jos\'ee Cros \and Sylvain Foissac \and  Annick Moisan \and C\'eline Noirot \and Damien Leroux \and Thomas Schiex \\ Applied Mathematics and Computer Science Dept.\\ INRA Toulouse, France}
...
\begin{abstract}
  ...
  organisms, and since version 4.0 also for prokaryotic organisms. It has been developed thanks to funding by INRA
...
\section{Annotating a prokaryote sequence}

Since version 4.0, \EuGene\ is able to annotate prokarotic sequence. \EuGene\ can especially predict overlapping genes and operons structure.
Here a simple example based on a \textit{Sinorhizobium meliloti} sequence.
To annotate prokaryotes, the -P flag must be used. Here we want to exploit similarities with 2 proteic databases, 
...
see section~\ref{protsim}.

\EuGene\ results shows that the two last genes are overlapping.
...
\item \texttt{EuGene.mode}: Prokaryote or Eukaryote. The ``\texttt{-P}'' command 
line flag activates the Prokaryote mode.  
...
\item \texttt{EuGene.InitExDist}, \texttt{EuGene.IntrExDist},
  ...
  \texttt{EuGene.RnaDist}, \texttt{EuGene.OverlapDist}, \texttt{EuGene.UIRDist}: \EuGene\ 
  ...
  5' UTR region, a 3' UTR region, a non coding protein RNA, an overlapping exons
  region or a untranslated intern region (UIR). Each parameter specifies the
...
the DNA codon table. The path is relative to EUGENEDIR. 
...
Extract of the default eukaryote codon table:
\begin{Verbatim}
AAA K	
AAC N
ACA T
...
Two genes predicted on the same strand whom distance is inferior to \texttt{Operon.maxDistance} 
...
If the ESTs are oriented, you can activate the parameter \texttt{AltEst.strandSpecific}.
...
The sum of all the absolute values of the parameters multiplied by this parameter is subtracted from the original fitness to define the final fiteness optimized.
\item \texttt{Eval.offset}: during the evaluation of a prediction, the prediction is compared with a 
