...
\newcommand{\EuGenie}{\textsc{EuG-bène}}-A
...
\author{Philipe Bardou, Marie-Jos-bée Cros, Sylvain Foissac, Annick Moisan,Thomas Schiex\\Biometry and AI Dept.\\ INRA Toulouse\\France}-A
...
started on sam d-béc 7 13:44:35 CET 2002-A
...
finished on sam d-béc 7 13:47:14 CET 2002-A
\end{Verbatim}
...
If you want to exploit similarities with cDNA/EST sequences, you have to inform \EuGenie\ of existing simiairities. These similarities
...
To be executed, \EuGenie\ needs at least one file: this is the
so-called parameter file.  \EuGenie\ behavior is entirely controlled
...
see below). Command line flags override any value in the parameter
file. The name of the parameter file that \EuGenie\ seeks is obtained
by adding the suffix ``\texttt{.par}'' to the name of the \EuGenie\ 
command itself.  As it is distributed, \EuGenie\ command's name is
...
\begin{itemize}
\item \texttt{EuGene.version}: specifies the version of the
...
\item \texttt{EuGene.InitExDist}, \texttt{EuGene.IntrExDist},
  ...
  \texttt{EuGene.5PrimeDist}, \texttt{EuGene.3PrimeDist}: \EuGenie\ 
  ...
  filename of an explicit penalty distribution file. %TBD
...
methods:
\begin{itemize}
\item creator: when instanciated, a plugin receives an instance number
  ...
  plugin depends on optimizable parameters (parameter whose name is
...
\item \texttt{Init}: receives as argument the sequence to process (an
  ...
  initializations that depends on optimizable parameter values
  (parameter whose name is followed by a \texttt{*}).
...
parameter file with a name finishing with an '*') migth be optimized
on a reference set of sequences (with their related information) for
which genes positions are known. The idea is to adapt the values of
parameters to increase as much as possible the prediction of genes and
exons. The figure \ref{fig:ParaOptimization} details the general
functioning of the software with input and ouput files.
...
with the \texttt{ParaOptimization.Use} parameter set to \texttt{TRUE}.


After updating the parameter file eugene.par (which sensors to
use,...), the software is lauched with the usual command line
specifying in argument the sequences to consider. At the end, the
software creates a new parameter file called
\texttt{eugene.<date>.OPTI.par} (for example,
\texttt{eugene.30Sep2003.OPTI.par}) with the new value for the
