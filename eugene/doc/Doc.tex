...
\author{Philipe Bardou, Marie-Jos\'ee Cros, Sylvain Foissac, Annick Moisan,Thomas Schiex\\Biometry and AI Dept.\\ INRA Toulouse\\INRA France}
...
\begin{abstract}
  ...
  ministry of research (with one PhD student). It is written in C++,
  generates text, HTML and graphical output.
  ...
  states) and covers UTR, UTR introns\ldots each with a possible explicit
  ...
  integrate prediction from other gene predictors if needed.
  
  In orer to integrate all this information, \EuGene\ does not use
...
\EuGene\ (the \emph{Arabidopsis thaliana} instanciation of \EuGene).
...
If you want to exploit similarities with cDNA/EST sequences, you have to inform \EuGene\ of existing simiairities. These similarities
...
\section{\EuGene's parameters}
\label{param}

Here is a list of all the parameters which control \EuGene's
...
\subsection{Specification of explicit penalty distributions on length}
...
variation of the penalty. Froma proabilitics point of view, this means
an exponential tail.
...
(linear interpolation), then 4.0 at 6. Because this is the last point
and the slope is 1, then the rest of the distribution will be linear
with slope 1.
...
\item \texttt{PostAnalyse}: receives as argument the prediction of
  \EuGene\ and may check it agains its own prediction and report
