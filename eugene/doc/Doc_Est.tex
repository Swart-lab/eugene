...
\item \texttt{Est.SpliceStartP} is the penalty affected if a spliced start 
codon is detected on the frontier of the first and the second match region 
in a genomic spliced alignment. Note that the minimum length of the first 
coding exon has to be 1 nt to allow the detection of spliced start. (check 
the value in \texttt{EuGene.InitExDist} file)
