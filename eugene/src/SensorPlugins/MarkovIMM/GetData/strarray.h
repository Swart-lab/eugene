...
class  String_Array
  ...
  public:
    ...
    void  Read  (FILE *);
    void  Read  (char []);
    void  BinRead  (FILE *);    // <Gloup> added
    void  Set  (double = 0.0);
    void  Set_All  (double = 0.0);
    void  Set_Lambda  (String_Array &);
    void  Set_Single  (String_Array * [], String_Array * []);
    int  Value  (char);
    int   AntiValue  (char);    // Tom added
    void  Write  (FILE *);      // <Gloup> added
    void  Write  (char []);
    void  BinWrite  (FILE *);   // <Gloup> added
...
// <Gloup> modified --> just call polymorph
...
void  String_Array :: BinRead  (FILE * fp)

//  <Gloup> added : read in binary format

  {
   ...
   M = A = N = 0;
   (void) fread (&M, sizeof(int), 1, fp);
   (void) fread (&A, sizeof(int), 1, fp);
   (void) fread (&N, sizeof(int), 1, fp);

   assert (M == Max_Str_Len && A == Alphabet_Size && N == Num_Entries);

   for  (i = N = 0;  i < Num_Entries;  i ++) {
       N += fread (&buffer, sizeof(unsigned short), 1, fp);
       Val [i] = ((float) buffer) * BINARY_PRECISION;
   }
...
void  String_Array :: Write  (FILE *fp)

//  Write the values of this  String_Array  to the file fp
...
//  <Gloup> added : binary version of Write
